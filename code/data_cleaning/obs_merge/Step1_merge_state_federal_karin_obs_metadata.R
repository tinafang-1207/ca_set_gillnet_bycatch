...
# Format Karin key
karin_key <- karin_key_orig %>% 
  # Omit 1990 data (in CDFW data)
  mutate(year=lubridate::year(date)) %>% 
  filter(year<1990) %>% 
  # Add dataset
  mutate(dataset="CDFW (Karin Forney)") %>% 
  # Simplify
  select(-c(strata, port_code, location, target_spp_code, net_length_obs_fa)) %>% 
  # Format port
  rename(port_depart=port) %>% 
  mutate(port_return=port_depart) %>% 
  # Add trip id
  mutate(trip_id=paste(vessel_id, date, sep="-")) %>% 
  # Add net type
  mutate(net_type="set") %>% 
  # Assume that missing target species is halibut
  mutate(target_spp=ifelse(is.na(target_spp), "California halibut", target_spp)) %>% 
  # Calculate average depth
  mutate(depth_fa=(depth_fa_min + depth_fa_max) / 2) %>% 
  select(-c(depth_fa_min, depth_fa_max))

# Merge keys
set_key_orig <- bind_rows(cdfw_key, swfsc_key, karin_key) %>% 
  # Add date things
  mutate(year=lubridate::year(date),
...
# 1b. Find block for sets with coords
################################################################################
...
# Median GPS coordinate by block
block_gps_key <- set_key_gps1 %>% 
  group_by(block_id) %>% 
  summarise(lat_dd=median(lat_dd),
            long_dd=median(long_dd)) %>% 
  ungroup()


# 1c. Find block for sets without coords and give block coords
################################################################################
...
blocks_df <- blocks %>% 
  ...
  rename(lat_dd2=block_lat_dd, long_dd2=block_long_dd) %>% 
  # Add observer GPS coordinates
  left_join(block_gps_key, by="block_id")
...
# Most commonly visited block on a date
block_key0 <- logs_use %>%
  # Summarize
...
set_key_no_gps1 <- set_key_no_gps %>%
  # Karin's data has block ids already
  rename(block_id0=block_id) %>% 
  # Convert vessel id
  ...
  # Override with provided block
  mutate(block_id=ifelse(!is.na(block_id0), block_id0, block_id),
         gps_type=ifelse(!is.na(block_id0), "Reported block centroid", gps_type)) %>% 
  # Move block id
  relocate(block_id0, .before=block_id1) %>% 
  # Add coordinates
  select(-c(long_dd, lat_dd)) %>% 
  left_join(blocks_df) %>% 
  # Select observer lat/long over centroid lat/long
  mutate(long_dd=ifelse(is.na(long_dd), long_dd2, long_dd),
         lat_dd=ifelse(is.na(lat_dd), lat_dd2, lat_dd)) %>% 
  select(-c(long_dd2, lat_dd2)) %>% 
  # Convert vessel id
  mutate(vessel_id=as.numeric(vessel_id)) %>% 
  # Remove
  select(-c(block_id0, block_id1:block_id5))


# 1d. Merge data with and without coordinates
################################################################################
...
soak_key1 <- set_key2 %>% 
  ...
  slice(1) %>% 
  ungroup() %>% 
  rename(soak_hr_mode1=soak_hr)

# Soak hour key by net and target species
soak_key2 <- set_key2 %>% 
  # Data with soak hours
  filter(!is.na(soak_hr)) %>% 
  group_by(net_type, target_spp, soak_hr) %>% 
  summarize(nsets=n()) %>% 
  ungroup() %>% 
  ...
  slice(1) %>% 
  ungroup() %>% 
  rename(soak_hr_mode2=soak_hr)

# Impute soak time
set_key3 <- set_key2 %>% 
  # Add soak time from source 1
  left_join(soak_key1 %>% select(-nsets), by=c("vessel_id", "net_type", "target_spp")) %>% 
  # Add soak time from source 1
  left_join(soak_key2 %>% select(-nsets), by=c("net_type", "target_spp")) %>% 
  # Impute
  mutate(soak_hr_type=ifelse(!is.na(soak_hr), "Reported", 
                             ifelse(!is.na(soak_hr_mode1), "Imputed-vessel mode", "Imputed-gear mode")),
         soak_hr=ifelse(!is.na(soak_hr), soak_hr, 
                        ifelse(!is.na(soak_hr_mode1), soak_hr_mode1, soak_hr_mode2)))

# Plot
plot(soak_hr~soak_hr_mode1, set_key3)
...
set_key4 <- set_key3 %>% 
  # Simplify
  select(-c(soak_hr_mode1, soak_hr_mode2)) %>% 
  # Add mesh size
